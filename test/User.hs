...
userCreate :: A.Handler -> (Port -> ClientEnv) -> SpecWith Port
userCreate h clientEnv =
        ...
        context "Avatar correct name" $ do
          it "responds with the correct user id written in the database, and saves an avatar" $ \port -> do
            ...
            let incomingUser =
                ...
                fd = FileData "avatar" "test/testImages/image0.jpg" "image/jpg" fileBytes :: FileData Mem
            created <- sendUser h incomingUser (Just fd) authInfo clientEnv port
            compareExpectedAndCreated h incomingUser (Just ("image/jpg", "test/testImages/image0.jpg")) created
        ...
        context "Avatar incorrect name" $ do
          it "Ignores the file and responds with the correct user id written in the database" $ \port -> do
            ...
            let incomingUser =
                ...
                fd = FileData "NotAvatar" "test/testImages/image0.jpg" "image/jpg" fileBytes :: FileData Mem
            created <- sendUser h incomingUser (Just fd) authInfo clientEnv port
            compareExpectedAndCreated h incomingUser Nothing created
...
userToAuthor :: A.Handler -> (Port -> ClientEnv) -> SpecWith Port
userToAuthor h clientEnv =
  describe "POST /user/toAuthor" $ do
    ...
    context "Incorrect auth" $ do
      ...
      it "Responds with 404" $ \port -> do
        void $ hRunDB (hDBHandler h) $ P.insert testU
        runClientM (toAuthor authInfo "test") (clientEnv port) >>= \case
          Left (FailureResponse _ fResp) ->
            if (statusCode . responseStatusCode $ fResp) == 404
              then pure ()
              else expectationFailure "Server didn't respond with 404"
          Left e -> throwIO e
          _ -> expectationFailure "Server didn't respond with an error"

userGetU :: A.Handler -> (Port -> ClientEnv) -> SpecWith Port
userGetU h clientEnv =
...
compareExpectedAndCreated :: A.Handler -> IncomingUserInfo -> Maybe (String, FilePath) -> User -> Expectation
